...
namespace Declick\CoreBundle\EventListener;
...
class UserListener implements EventSubscriberInterface {
    ...
    public function onRegistrationSuccess(FormEvent $event) {
        $url = $this->router->generate('declick_user_registration_confirmed');
        $event->setResponse(new RedirectResponse($url));
    }

    public function onChangePasswordSuccess(FormEvent $event) {
        $url = $this->router->generate('declick_user_profile_show');
        $event->setResponse(new RedirectResponse($url));
    }
    
    public function onResettingPasswordSuccess(FormEvent $event) {
        $url = $this->router->generate('declick_user_password_reset_confirmed');
        $event->setResponse(new RedirectResponse($url));
    }
