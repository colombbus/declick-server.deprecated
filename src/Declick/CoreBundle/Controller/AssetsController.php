...
class AssetsController extends Controller {
    ...
    protected function checkDirectory($project) {
        $uploadPath = $this->container->getParameter('declick_core.settings.directory.upload');
    ...
    protected function checkEnvironment($fields, $xmlCheck = true, $projectId = false) {
        ...
        $project = $this->getDoctrine()
                ->getManager()
                ->getRepository('DeclickCoreBundle:Project')
        ...
        $env->projectPath = $this->get('declick_core.project_manager')->getProjectPath($project);
    ...
    public function getResourcesAction() {
        ...
        $resources = $this->get('declick_core.project_manager')->getAllResources($env->project);
    ...
    public function getProgramsAction() {
        ...
        $programs = $this->get('declick_core.project_manager')->getAllPrograms($env->project);
    ...
    protected function getProgramContent($statements = false) {
        ...
        $program = $this->get('declick_core.project_manager')->getProjectFile($env->project, $programName, true);
        if ($program === false) {
            return $jsonResponse->setData(array('error' => 'program_not_found'));
        }

        $filePath = $this->get('declick_core.file_manager')->getFilePath($program);
    ...
    public function getResourceAction($projectId, $version,$name) {
        ...
        $resource = $this->get('declick_core.project_manager')->getProjectFile($env->project, $name, false);
        if ($resource === false) {
        ...
        if (is_int($version) || $version != $currentVersion) {
            ...
            if ($projectId !== false) {
                return $this->redirect($this->generateUrl( 'declick_get_resource_1', array('name'=>$name, 'projectId'=>$projectId, 'version'=>$currentVersion) ));
            } else {
                return $this->redirect($this->generateUrl( 'declick_get_resource_2', array('name'=>$name, 'version'=>$currentVersion) ));
            }
        }
        
        $path = $env->projectPath."/".$this->get('declick_core.file_manager')->getFilePath($resource);
    ...
    public function removeProgramAction() {
        ...
        $pManager = $this->get('declick_core.project_manager');
        // get program
        $manager = $this->get('declick_core.file_manager');
    ...
    public function addResourceAction() {
        ...
        foreach ($files as $uploadedFile) {
            ...
            // Get new file
            $file = $this->get('declick_core.project_manager')->createFile($env->project, $name, false);
            if ($file === false) {
    ...
    public function renameProgramAction() {
        ...
        $existing = $this->get('declick_core.project_manager')->isProjectFile($env->project, $newName, true);
        if ($existing) {
            return $jsonResponse->setData(array('error' => 'program_already_exists'));
        }
    ...
    public function createResourceAction() {
        ...
        // Create resource
        $resource = $this->get('declick_core.project_manager')->createFile($env->project, $name, false);
        if ($resource === false) {
            // file name already in use
            return $jsonResponse->setData(array('error' => 'resource_already_exists'));
        }        
